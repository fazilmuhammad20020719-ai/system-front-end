...
import { Plus, Calendar, Clock, ChevronRight, BookOpen, Trash2 } from 'lucide-react';
                        ...
                        >
                            <div>
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <button onClick={(e) => handleDeleteSlot(e, slot.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
                {slotExams.map(exam => (
                    <div key={exam.id} className="bg-white p-4 rounded-lg border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 hover:shadow-sm transition-shadow">
                        <div className="flex items-center gap-4">
                            <div className="bg-green-50 text-green-700 w-12 h-12 rounded-lg flex flex-col items-center justify-center font-bold border border-green-100">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500">{exam.subject_name} • {exam.start_time} - {exam.end_time}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="text-right text-sm text-slate-500 mr-4">
                                <p className="font-bold text-slate-700">{exam.venue || 'No Venue'}</p>
                                <p>{exam.total_marks} Marks</p>
                            </div>
